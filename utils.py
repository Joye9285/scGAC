...
# retain the top NR_OF_KNN neighbors
...
"""
def getGraph(dataset_str, features, L, K, method, NEsh=0.5):
    print(method)
...
"""
def load_data(dataset_str, L, K, param, PCA_dim, method='NE', NEsh=0.6, n_clusters=20):
    # Get data
    ...
    if method == 'all':
        adj_ps = getGraph(dataset_str, features, L, K, 'pearson')
        adj_sp = getGraph(dataset_str, features, L, K, 'spearman')
        adj = adj_ps * adj_sp
    else:
        adj = getGraph(dataset_str, features, L, K, method, NEsh)
    ...
    rowsum = None
    return adj, features, rowsum, cells, genes
...
def my_kmeans(K, hidden, dataset_str):
    ...
    for i in range(len(labels)):
        result.append([cells[i],labels[i]])
    result = pd.DataFrame(np.array(result),columns=['cell','label'])
    result.to_csv(pred_path, index=False, sep='\t')
